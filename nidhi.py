...
# Video caption processing – new rules
def process_caption(text: str, numbering: str) -> str:
    # 1. Detect "Title" (case‑insensitive) then the first number after it,
    #    and remove everything before that number including itself.
...
# PDF/HTML renaming – new rule: detect number in filename, remove everything before that number including itself
def remove_leading_number(filename: str) -> str:
    name, ext = os.path.splitext(filename)
    # Remove everything from start up to (and including) the first number + optional spaces
    ...
    if message.video:
        ...
        if fname.lower().endswith((".pdf", ".html")):
            ...
            try:
                ...
                pass
            # Rename file using the new rule
